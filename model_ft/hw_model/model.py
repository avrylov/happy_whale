...
class SNUNet(nn.Module):
    def __init__(self, in_ch=3, out_ch=2):
        ...
        self.regression = torch.nn.Sequential(
            torch.nn.Linear((1 * 128 * 128), 1024),
            torch.nn.ReLU(),
    ...
    def forward(self, xA, xB):
        ...
        out_shape = out.size(1) * out.size(2)
        out_vector = out.view(out.size(0), out_shape)

        prob = self.regression(out_vector)
...
if __name__ == '__main__':
    ...
    model = SNUNet().to(device)

    print(
        summary(
            model, [(3, 128, 128), (3, 128, 128)]
        )
    )